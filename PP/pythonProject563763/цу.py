...
ax.grid(which='minor',
        color='k',
        linestyle=':')
def fp_length(average_lenght):  # вычисление длины свободного пробега исходя из ФР
    rng = random.uniform(0, 1)
    lenght = -average_lenght * log(rng, e)
    return lenght
...
def count_sect_current():
    ...
    for i in range(10):
        ...
        if ((temp1 >= y0_sect and temp1 <= y1_sect) and (x_wall_left < particle.x0 and particle.x1 <= x_wall_left)) or (
...
def count_target_current():
    ...
    # if particle.y1 > z_collector:
    #     st = str(round(z_collector))+" - "+str(round(particle.y0))
    #     canvas.create_text(particle.x1, particle.y1, text=st, justify=CENTER, font="Verdana 12")
    if (temp1 >= target_left and temp1 <= target_right) and (z_collector > particle.y0 and particle.y1 > z_collector):
        target_current = target_current + 1
        # canvas.create_text(particle.x1, particle.y1, text=target_current, justify=CENTER, font="Verdana 12")
# -----------------------------  END  ----------------------------------
def dist(wall, lamda, x_plane):
    ...
    k = 1
    while k <= k_max:
        particle.x0 = diam_cam / 2 - random.randint(- apertura / 2, apertura / 2)  ##apertura/2 + (k // (k_max/n_int)) * apertura/n_int
...
temp_cur = 0
# ------------------------  Режим номер 1  ------------------------
def current_free_and_tube(): # Расчет тока на мишень при наличии ограничивающией трубки и в её отсутствии
    global illustration
    ...
    for f in range(n_del):
        ...
        # x[f] = 133*0.358/((average_lambda + f*average_lambda)/k_px)
...
def longitudinal_distribution(): # распределение тока по секциям
    ...
    for t in range(n_del):
        ...
        with open("data.txt", "a") as file:
            ...
            # y[n_sect] = target_current / sum_cur
        # str_f = str(round(x[f], 3)) + " " + str(round(y[f], 3))
        # with open("data.txt", "a") as file:
        #     print(str_f, file=file, sep="\n")
